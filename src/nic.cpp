...
void NIC::StatInBandwidth(obd_count count)
{
	if (__stat) {
		...
		if (ticks >= params.TimeUnit) {
			...
			bw = ibytes * params.TimeUnit / params.SizeUnit / ticks;
			st[IN]->Record("%llu.%09llu %llu\n",
...
void NIC::StatOutBandwidth(obd_count count)
{
	if (__stat) {
		...
		if (ticks >= params.TimeUnit) {
			...
			bw = obytes * params.TimeUnit / params.SizeUnit / ticks;
...
void NIC::InitStat()
{
	if (__stat) {
		...
		sprintf(str, "%s-%dnic.in", name, nid);
		st[0] = new Stat(str);
		sprintf(str, "%s-%dnic.out", name, nid);
		st[1] = new Stat(str);
